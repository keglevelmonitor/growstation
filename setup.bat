...
:: setup.bat - Bootstrap installer for GrowStation (Windows)
:: Can be run from anywhere (e.g. after: curl -sL ... -o setup.bat && setup.bat)
:: Clones repo to %USERPROFILE%\growstation and runs install.bat

SET "INSTALL_DIR=%USERPROFILE%\growstation"
SET "DATA_DIR=%USERPROFILE%\growstation-data"
SET "SHORTCUT_PATH=%USERPROFILE%\Desktop\GrowStation.lnk"
SET "REPO_URL=https://github.com/keglevelmonitor/growstation.git"

TITLE GrowStation Auto-Installer

echo.
echo ========================================
echo    GrowStation Windows Installer
echo ========================================
echo.
...
SET "WHAT_TO_INSTALL=GrowStation Application and Data Directory"
...
IF /i "%choice%"=="UPDATE" (
    SET "WHAT_TO_INSTALL=GrowStation Update"
    SET "CLEANUP_MODE=NONE"
    goto :confirm_proceed
)
IF /i "%choice%"=="APP" (
    SET "WHAT_TO_INSTALL=GrowStation Application (Fresh App, Keep Data)"
    SET "CLEANUP_MODE=APP"
    goto :confirm_proceed
)
IF /i "%choice%"=="ALL" (
    SET "WHAT_TO_INSTALL=GrowStation Application and Data Directory (Fresh Install)"
